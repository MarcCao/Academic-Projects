/*
    ...
    using Jacobi iterations and MPI.  For simplicity we will assume that we will use a uniform discretization.
*/
...
int main(int argc, char *argv[])
{
    ...
    // Handle the case of only one process 
    if (num_procs == 1)
    {
    ...
    // For reference, (x_i, y_j) u[i][j]， so that i references columns and j rows
    ...
    // Inital copy into u_old - note that this does not require communication， as we know all values on each process at this point
    for (int i = 0; i < N + 2; ++i)
        for (int j = 0; j < rank_N + 2; ++j)
            u_old[i][j] = u[i][j];
    ...
    // Each rank writes out to it's own file and post-processing will handle opening up all the files - rank determines the file names
